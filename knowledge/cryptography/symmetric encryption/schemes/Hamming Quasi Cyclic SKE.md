...
> 	1. Generates $h \xleftarrow{\$} \mathbb F_2^n$
> 	2. Calculate $s \leftarrow x + h \cdot y$
> 	3. Generates $e \xleftarrow{\$} S_{w_e}^n$
> 	4. Generates $r = (r_1, r_2) \xleftarrow{\$} S_{w_r}^n(\mathbb F_2) \times  S_{w_r}^n(\mathbb F_2)$
> 	5. $u = r_1 + h \cdot r_2$
> 	6. $v = mG + s \cdot r_2 + e$
> 	7. Return $(u, v)$
...
> 	3. $sk = (x, y) \xleftarrow{\$} S_w^n(\mathbb F_2) \times S_w^n(\mathbb F_2)$, $\cdot$ is polynomial multiplication mod $X^n - 1$
> 	4. $pk = (h, s = x + h \cdot y) \in \mathbb F_2^n \times \mathbb F_2^n$
> 	5. returns $(h, s, y)$
> - $c \leftarrow \text{Enc}(k, m)$:
> 	1. Generates $h \xleftarrow{\$} \mathbb F_2^n$
> 	2. Calculate $s \leftarrow x + h \cdot y$
