...
### Against Affine Prover Strategies Variant

> [!definition] Knowledge Soundness
> Given a [[Non-Interactive Linear Proofs]] $\Pi_\mathsf{NILP}$. For any adversary $\mathcal{A} = (\mathcal{A}_\mathsf{find})$, there exists an efficient extractor $\mathcal{E} = (\mathcal{E}_\mathsf{find})$, we define the knowledge soundness against prover strategies advantage:
> $$\mathsf{Adv}_\mathsf{NIPS}^{\mathsf{ks}}(\mathcal{A}, \mathcal{E}) = 
\Pr\!\left[ 
\begin{array}{l}
\Pi \in \mathbf{F}^{k \times m} \\
(\mathbf{x}, \mathbf{w}) \notin \mathcal{R}\\
\mathsf{Verify}(\mathcal{R}, st, \mathbf{x}, \Pi^* \mathrm{crs}) = 1
\end{array} 
\;\middle |\; 
\begin{array}{l}
(\mathrm{crs}, \mathrm{st}) \leftarrow \mathsf{Setup}(1^\lambda, \mathcal{R}) \\
(\mathbf{x}, \Pi^*) \leftarrow \mathcal{A}_\mathsf{find}(\mathcal{R}, \mathrm{crs}, \mathrm{st}) \\
\mathbf{w} \leftarrow \mathcal{E}_\mathsf{find}(1^\lambda, \mathcal{R}, \mathrm{crs}, \mathrm{st}, \mathbf{x}, \Pi^*)
\end{array} \right]$$
