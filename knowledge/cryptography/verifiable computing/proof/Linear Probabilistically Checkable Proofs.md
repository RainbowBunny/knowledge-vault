...
>  $$
> \Pr\!\left[ \mathsf{Verify}(st, x, Q^T \boldsymbol{\pi}) = 1 \;\middle |\; 
> \begin{array}{l}
...
> [!definition] (Statistical) Soundness
> For all $x \in \mathbb F^n$ such that $\mathcal{CS}(\mathbf{x}, \mathbf{w}) = 0$ for all $w \in $
