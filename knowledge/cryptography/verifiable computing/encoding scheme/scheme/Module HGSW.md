...
> - $\mathbf{C}_i \leftarrow \mathsf{Encrypt}(i, \mathrm{sk}, \boldsymbol{\mu}) \in R_q^{m_q \times (k + \ell')}$: Given the message index $i$, secret key $\mathrm{sk}$ and a message vector $\boldsymbol{\mu}_i^T = (\mu_{i, 1}, \dots, \mu_{i, \ell}) \in R_p^\ell$, computes the ciphertext.
...
> - $c^{*} \leftarrow \mathsf{Add}(\{\mathbf{C}_i\}_{i \in [m]}, \{a_i\}_{i \in [m]}) \in R_q^{1 \times (k + \ell')}$:
...
> - $\mathsf{Decrypt}(\mathbf{S}, \mathbf{c}^*)$: Given a ciphertext $\mathbf{c}^*$ and the secret key $\mathsf{sk}$.
