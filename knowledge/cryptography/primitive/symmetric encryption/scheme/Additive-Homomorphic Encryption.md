...
> 	4. Compute $B = \mathrm{Mat}(b) = \begin{pmatrix}f_1 & f_2 &\cdots g_{m - w}\end{pmatrix}$ (Basis Matrix).
> 	5. Defines $D$ as the last $m - w$ columns of its transposed inverse $(B^{-1})^T$ (Dual Basis)
> 	6. Samples $s \xleftarrow{\$} F^n$ with $F = \mathrm{supp}(f)$.
> 	7. Return $k = (f, g, D, s)$.
> - $c \leftarrow \mathrm{Encrypt}(k, m, r)$:
> - $m \leftarrow \mathrm{Decrypt}(k, c)$:
