...
Strict inclusions follow from the [[Hierarchy Theorems|time and space hierarchy theorems]]; equalities like $\text{PSPACE} = \text{NPSPACE}$ follow from [[Space Complexity#Savitch's Theorem|Savitch's theorem]] and $\text{IP} = \text{PSPACE}$ from Shamir's theorem (see [[Class IP|Interactive Proofs]]).
