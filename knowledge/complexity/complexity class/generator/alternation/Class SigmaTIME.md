...
> [!definition] Class $\Sigma$TIME
> For every $i \in \mathbb{N}$, we define $\Sigma_i \mathsf{TIME}(T(n))$ to be the set of [[Language]] accepted by a $T(n)$-time [[Alternating Turing Machine]] $M$ whose initial state is labeled "$\exists$" and on which every input and on every (directed) path from the starting configuration in the configuration graph, $M$ can alternate at most $i - 1$ times from states with one label to states with other label.
