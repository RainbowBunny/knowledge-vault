...
> [!remark]
> Minimum distance and error-correction capability are defined in [[Code Distance]]; for linear codes $d(C) = \text{wt}(C)$ by the theorem above. In code-based cryptography the Hamming metric is often generalized: for a code over a ring $\mathcal R$ with norm $\omega$, one sets $d = \min_{u \neq v \in C} \omega(u - v)$ (see the remark in [[Code Distance]]).
