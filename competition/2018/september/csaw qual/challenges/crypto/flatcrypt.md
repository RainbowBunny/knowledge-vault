...
---
Idea:
Because we are having `PROBLEM_KEY + f` compressed by `zlib` and `PROBLEM_KEY` are in lowercase letters and underscore and `len(PROBLEM_KEY + f) > 20` so we can use `PADDING + CANDIDATE + KNOWN_FLAG + PADDING` where `PADDING` is something not from the set of flag.
